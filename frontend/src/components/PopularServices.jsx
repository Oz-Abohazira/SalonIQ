            ...
            <div className='w-full grid grid-cols-auto gap-6 pt-5 gap-y-6 px-3 sm:px-0'>
                ...
                {servicesData.filter(service => popularServices
                    ...
                    .map((item, index) => {
                        ...
                        return (
                            <div onClick={() => { navigate(`/appointment/${item._id}`); scrollTo(0, 0) }} className='border border-blue-200 rounded-xl max-w-70 overflow-hidden cursor-pointer
                                    hover:-translate-y-2.5 transition-all duration-500' key={index}>
                                <img className='bg-blue-50 h-70' src={item.image} alt="" />
                                <div className='p-4'>
